"""Real Chromium + authenticated HTTP JSON import + four TCP sockets.
Optional Playwright dependency. No mocked requests and no browser-policy changes.
"""
...
def main():
    ...
    peers=[];report={'passed':False,'mode':'live browser HTTP and TCP','checks':[]}
    with tempfile.TemporaryDirectory() as temp:
        ...
        try:
            ...
            def send(index,project,station,**fields):
                ...
                sock.sendall(encode({'v':2,'type':'result','msg_id':uuid.uuid4().hex,'project':project,'station':station,'worker_id':f'D70{516+index}',**fields}))
                reply=decode(stream.readline()[:-1]);assert reply['recorded']
            with sync_playwright() as pw:
                ...
                report['checks'].append('two live screw stations')
                page.locator('[data-project=packaging]').click();expect(page.locator('#standard-file')).to_be_enabled()
                expect(page.locator('#save-standard')).to_be_disabled()
                def choose(barcode):
                    page.locator('#standard-file').set_input_files({'name':'standard.json','mimeType':'application/json',
                        'buffer':json.dumps({'standard_barcode':barcode}).encode()})
                    expect(page.locator('#save-standard')).to_be_enabled()
                choose('001234-AbC')
                with page.expect_response(lambda r:r.url.endswith('/standard-barcode/import') and r.request.method=='POST') as saved:
                ...
                report['checks'].append('JSON import, barcode match and independent LOGO/flame')
                choose('UNSAVED-000');page.wait_for_timeout(1800)
                expect(page.locator('#standard-barcode')).to_have_value('UNSAVED-000')
                me=page.request.get(base+'/api/auth/me').json()
                response=page.request.post(base+'/api/admin/standard-barcode/import',
                    data={'content':json.dumps({'standard_barcode':'NEW-STANDARD'}),'revision':1},
                    headers={'Origin':base,'X-CSRF-Token':me['csrf'],'X-Request-ID':uuid.uuid4().hex})
                assert response.status==200
                with page.expect_response(lambda r:r.url.endswith('/standard-barcode/import') and r.request.method=='POST') as conflict:
                    page.locator('#save-standard').click()
                assert conflict.value.status==409
                report['checks'].append('pending file survives polling; stale revision rejected')
                hostile='<img src=x onerror=alert(1)>';send(3,'packaging',2,barcode=hostile,logo='OK',flame='NG')
                expect(page.locator('#records-body')).to_contain_text(hostile);assert page.locator('#records-body img').count()==0
                ...
                report['checks'].append('safe barcode rendering and rapid page switches')
                ...
                report['checks'].append('authenticated CSV export')
                public=browser.new_context();board=public.new_page();board.goto(base+'/board/')
                ...
                data=board.request.get(base+'/api/display?project=packaging').json();assert 'standard' not in data and 'logs' not in data
                assert public.request.get(base+'/api/admin/standard-barcode/template').status==401
                report['checks'].append('public view cannot access standard/import controls')
                ...
                report['checks'].append('disconnect preserves history')
                assert not errors,errors;report['checks'].append('no JavaScript errors');report['passed']=True
